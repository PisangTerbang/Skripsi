<x-layout-kalab>
    <x-slot:title>Pengaturan Akun</x-slot>
    ...
    <div x-data="pengaturanPage()" class="space-y-6">
        ...
        <div
            ...
            <div class="relative flex items-center gap-5">
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-widest text-sky-300">Pengaturan Akun</p>
                    <h2 class="mt-1 text-2xl font-black text-white leading-tight">{{ $user->name }}</h2>
                    <p class="mt-1 text-sm text-sky-200">{{ $user->email }}</p>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-4">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    <div
                        class="flex items-center gap-3 border-b-4 border-sky-200 bg-gradient-to-r from-sky-600 to-blue-700 px-5 py-4">
                        <div
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="rounded-xl border-2 border-sky-100 bg-sky-50 px-4 py-3">
                            <p class="text-xs font-bold uppercase tracking-widest text-sky-400 mb-1">Role</p>
                            <p class="text-sm font-black text-sky-800">Kepala Laboratorium</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-sky-200">Perbarui nama dan email Anda</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('ka-lab.pengaturan.profile') }}" enctype="multipart/form-data"
                        class="p-6 space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="flex items-center gap-5 rounded-xl border-2 border-gray-100 bg-gray-50 p-4">
                            <div class="relative shrink-0">
                                @if ($user->avatar)
                                    <img src="{{ asset('storage/' . $user->avatar) }}" alt="Avatar"
                                        class="h-16 w-16 rounded-full border-4 border-sky-100 object-cover shadow-sm">
                                ...
                                @else
                                    <div
                                        class="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-blue-600 text-xl font-black text-white shadow-sm border-4 border-sky-100">
                                        {{ strtoupper(substr($user->name, 0, 2)) }}
                                    </div>
                                @endif
                            </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $user->name) }}" required
                                class="w-full rounded-xl border-2 border-gray-200 px-4 py-3 text-sm text-gray-800 focus:border-sky-400 focus:outline-none focus:ring-2 focus:ring-sky-100 transition
                        ...
                        <div class="border-t-2 border-gray-100 pt-4">
                            <button type="submit"
                                class="inline-flex items-center gap-2 rounded-xl border-2 border-sky-300 bg-sky-600 px-6 py-3 text-sm font-black text-white shadow-sm transition hover:bg-sky-700 hover:shadow-md">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    ...
                    <form method="POST" action="{{ route('ka-lab.pengaturan.password') }}" class="p-6 space-y-5">
                        @csrf
                        @method('PUT')
            ...
            function pengaturanPage() {
                return {
                    ...
                    async removeAvatar() {
                        ...
                        try {
                            const res = await fetch("{{ route('ka-lab.pengaturan.avatar.remove') }}", {
                                method: 'DELETE',
                                headers: {
