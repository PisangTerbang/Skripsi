<x-layout-kalab>
    ...
    {{-- ✅ x-data membungkus SEMUA konten termasuk modal --}}
    <div x-data="validasiPage()" x-init="init()">
        ...
        <div class="min-h-screen bg-slate-100">
            <div class="px-6 py-6 space-y-6">
                ...
                {{-- ===== TOP BAR ===== --}}
                <div class="sticky top-0 z-10 border-b-2 border-yellow-100 bg-white px-6 py-4 shadow-sm -mx-6 -mt-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-xl border-2 border-yellow-200 bg-yellow-50">
                                <x-heroicon-o-clipboard-document-check class="h-5 w-5 text-yellow-600" />
                            </div>
                            <div class="h-8 w-px bg-gray-200"></div>
                            <div>
                                <h1 class="text-lg font-extrabold text-gray-900">Validasi Judul TA</h1>
                                <p class="mt-0.5 text-xs text-gray-400">Review dan validasi judul yang diajukan dosen
                                </p>
                            </div>
                        </div>
                        @if ($judulPending->count() > 0)
                            <span
                                ...
                                {{ $judulPending->count() }} menunggu validasi
                            </span>
                        @endif
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div
                        ...
                        <div class="relative flex items-start justify-between">
                            <div>
                                <p class="text-xs font-bold uppercase tracking-widest text-yellow-100">Menunggu Validasi
                                </p>
                                <p class="mt-3 text-5xl font-black leading-none text-white">{{ $judulPending->count() }}
                                </p>
                        ...
                        <div class="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-white/20">
                            <div
                                class="h-full {{ $judulPending->count() > 0 ? 'animate-pulse' : '' }} w-full rounded-full bg-white/60">
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="relative flex items-start justify-between">
                            <div>
                                <p class="text-xs font-bold uppercase tracking-widest text-emerald-200">Divalidasi</p>
                                <p class="mt-3 text-5xl font-black leading-none text-white">{{ $divalidasi }}</p>
                                <p class="mt-2 text-xs font-medium text-emerald-200">{{ $pctValidasi }}% dari total</p>
                            </div>
                            <div
                                class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border-2 border-white/20 bg-white/20">
                    ...
                    <div
                        ...
                        <div class="relative flex items-start justify-between">
                            <div>
                                <p class="text-xs font-bold uppercase tracking-widest text-violet-200">Total Peminat</p>
                                <p class="mt-3 text-5xl font-black leading-none text-white">{{ $totalPeminat }}</p>
                                <p class="mt-2 text-xs font-medium text-violet-200">mahasiswa</p>
                            </div>
                            <div
                                class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border-2 border-white/20 bg-white/20">
                    ...
                    <div
                        class="relative overflow-hidden rounded-2xl border-2 border-indigo-300 bg-gradient-to-br from-indigo-500 via-indigo-600 to-blue-700 p-6 shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl">
                        <div class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-white/10"></div>
                        <div class="absolute -bottom-6 -left-4 h-20 w-20 rounded-full bg-white/5"></div>
                        <div class="relative flex items-start justify-between">
                            <div>
                                <p class="text-xs font-bold uppercase tracking-widest text-indigo-200">Ditetapkan</p>
                                <p class="mt-3 text-5xl font-black leading-none text-white">{{ $totalDitetapkan }}</p>
                                <p class="mt-2 text-xs font-medium text-indigo-200">sudah ditetapkan</p>
                            </div>
                            <div
                                class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border-2 border-white/20 bg-white/20">
                                ...
                                <template x-for="(item, index) in paginatedData" :key="item.id">
                                    <tr class="group transition-colors hover:bg-yellow-50/30">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span
                                                    ...
                                                    x-text="(currentPage - 1) * perPage + index + 1"></span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span
                                        ...
                                        <td class="max-w-[220px] px-6 py-4">
                                            <p class="line-clamp-2 text-sm font-bold text-gray-800 leading-relaxed"
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-sky-500 to-blue-600 text-xs font-black text-white"
                                                    x-text="item.dosen.charAt(0).toUpperCase()"></div>
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                ...
                                                x-text="item.total_peminat"></span>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <span
                                                ...
                                                x-text="item.jumlah_ditetapkan"></span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span
                    ...
                    <div x-show="totalPages > 1 && filteredData.length > 0"
                        ...
                        <div class="flex items-center gap-1.5">
                            ...
                            <button @click="currentPage++" x-bind:disabled="currentPage === totalPages"
                                class="flex h-8 w-8 items-center justify-center rounded-xl border-2 border-gray-200 bg-white text-xs font-black text-gray-500
                                transition hover:border-yellow-300 hover:bg-yellow-50 hover:text-yellow-700 disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        <div x-show="showModal" x-cloak @click.self="showModal = false"
            ...
            <div x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95"
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="rounded-xl border-2 border-gray-100 bg-gray-50 p-3">
                            <p class="text-xs font-bold uppercase tracking-widest text-gray-400 mb-1">Dosen Pembimbing
                            </p>
                    ...
                    {{-- ✅ fix: hapus tanda kutip, pakai x-text langsung --}}
                    ...
                    <div x-show="selectedItem.status === 'pending_kalab'">
                        ...
                        <form method="POST" :action="'/ka-lab/validasi/' + selectedItem.id + '/approve'"
                        ...
                        <form method="POST" :action="'/ka-lab/validasi/' + selectedItem.id + '/reject'"
                            class="space-y-3">
                            @csrf
                        ...
                        <div class="rounded-xl border-2 p-4"
                            ...
                            <div class="flex items-center gap-2 mb-2">
                                <span
                                    ...
                                    <span class="h-1.5 w-1.5 rounded-full"
                                        x-bind:class="selectedItem.status === 'ditawarkan' ? 'bg-emerald-500' : 'bg-red-500'"></span>
                                    <span
