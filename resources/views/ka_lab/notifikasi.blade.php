<x-layout-kalab title="Notifikasi">

    <div x-data="notifikasiKaLab()" x-init="init()" class="min-h-screen bg-slate-100">
        <div class="px-6 py-6 space-y-6">

            {{-- ===== TOP BAR ===== --}}
            <div class="sticky top-0 z-10 border-b-2 border-blue-100 bg-white px-6 py-4 shadow-sm -mx-6 -mt-6">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div
                            class="flex h-10 w-10 items-center justify-center rounded-xl border-2 border-blue-200 bg-blue-50">
                    ...
                    <div class="flex items-center gap-2">
                        {{-- ✅ fix: x-bind:disabled dan x-bind:class --}}
                        ...
                        <button @click="markAllAsRead()" x-bind:disabled="loading || unreadCount === 0"
                            class="inline-flex items-center gap-2 rounded-xl border-2 border-blue-300 bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div class="flex items-center gap-1 rounded-2xl border-2 border-gray-200 bg-white p-1.5 shadow-sm w-fit">
                <button @click="filter = 'all'"
                    x-bind:class="filter === 'all' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                ...
                <button @click="filter = 'unread'"
                    x-bind:class="filter === 'unread' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                    class="rounded-xl px-4 py-2 text-xs font-bold transition-all">
                    Belum Dibaca
                    <span class="ml-1.5 rounded-full px-2 py-0.5 text-xs font-black"
                        x-bind:class="filter === 'unread' ? 'bg-white/20 text-white' : 'bg-blue-50 text-blue-600'">
                        <span x-text="unreadCount"></span>
                    </span>
                </button>
                <div class="h-5 w-px bg-gray-200"></div>
                <button @click="filter = 'read'"
                    x-bind:class="filter === 'read' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                    class="rounded-xl px-4 py-2 text-xs font-bold transition-all">
                    Sudah Dibaca
                    <span class="ml-1.5 rounded-full px-2 py-0.5 text-xs font-black"
            ...
            <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                ...
                <div
                    class="flex items-center justify-between border-b-4 border-blue-200 bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-4">
                    <div class="flex items-center gap-3">
                        <div
                    ...
                    <div>
                            ...
                            <div class="divide-y-2 divide-gray-100">
                                <template x-for="notif in filteredNotifications" :key="notif.id">
                                    <div @click="markAsRead(notif.id)"
                                        ...
                                        <div x-bind:class="!notif.is_read ? 'bg-blue-100 border-blue-200' : 'bg-gray-100 border-gray-200'"
                                            class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 transition-all group-hover:scale-105">
                                            <svg x-bind:class="!notif.is_read ? 'text-blue-600' : 'text-gray-500'"
                                        ...
                                        <template x-if="!notif.is_read">
                                            <div
                                                class="mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full bg-blue-600 shadow-sm shadow-blue-300">
                                            </div>
                                        </template>
                        ...
                        <template x-if="filteredNotifications.length === 0">
                            <div class="flex flex-col items-center justify-center py-24 text-center">
                                <div
                                    class="flex h-24 w-24 items-center justify-center rounded-3xl border-2 border-blue-100 bg-gradient-to-br from-blue-50 to-indigo-100 mb-6">
                                    <x-heroicon-o-bell-slash class="h-12 w-12 text-blue-300" />
                                </div>
    ...
    <div id="toast-container-kalab" class="fixed bottom-4 right-4 z-50 space-y-2"></div>

    @push('scripts')
        <script>
            function notifikasiKaLab() {
                return {
                    ...
                    async markAsRead(id) {
                        ...
                        this.readCount++;
                        try {
                            const res = await fetch(`/ka-lab/notifikasi/${id}/read`, {
                                method: 'POST',
                                credentials: 'same-origin',
                                headers: {
                    ...
                    async markAllAsRead() {
                        ...
                        this.loading = true;
                        try {
                            const res = await fetch("{{ route('ka-lab.notifikasi.read') }}", {
                                method: 'POST',
                                credentials: 'same-origin',
                                headers: {
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    ...
                    showToast(message, type = 'success') {
                        ...
                        document.getElementById('toast-container-kalab').appendChild(toast);
                        setTimeout(() => {
