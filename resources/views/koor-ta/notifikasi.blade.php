<x-layout-koor-ta title="Notifikasi">

    <div x-data="notifikasiPage()" x-init="init()" class="min-h-screen bg-slate-100">
        <div class="px-6 py-6 space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div
                            ...
                            <x-heroicon-o-bell class="h-5 w-5 text-indigo-600" />
                        </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button @click="markAllAsRead()" :disabled="loading || unreadCount === 0"
                            class="inline-flex items-center gap-2 rounded-xl border-2 border-indigo-300 bg-indigo-600 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div class="flex items-center gap-1 rounded-2xl border-2 border-gray-200 bg-white p-1.5 shadow-sm w-fit">
                <button @click="filter = 'all'"
                    :class="filter === 'all' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                    class="rounded-xl px-4 py-2 text-xs font-bold transition-all">
                    Semua
                    <span class="ml-1.5 rounded-full px-2 py-0.5 text-xs font-black"
                        :class="filter === 'all' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'">
                        <span x-text="totalCount"></span>
                    </span>
                </button>
                <div class="h-5 w-px bg-gray-200"></div>
                <button @click="filter = 'unread'"
                    :class="filter === 'unread' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                    class="rounded-xl px-4 py-2 text-xs font-bold transition-all">
                    Belum Dibaca
                    <span class="ml-1.5 rounded-full px-2 py-0.5 text-xs font-black"
                        :class="filter === 'unread' ? 'bg-white/20 text-white' : 'bg-indigo-50 text-indigo-600'">
                        <span x-text="unreadCount"></span>
                    </span>
                </button>
                <div class="h-5 w-px bg-gray-200"></div>
                <button @click="filter = 'read'"
                    :class="filter === 'read' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
                    class="rounded-xl px-4 py-2 text-xs font-bold transition-all">
                    Sudah Dibaca
                    <span class="ml-1.5 rounded-full px-2 py-0.5 text-xs font-black"
                        :class="filter === 'read' ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'">
                            ...
                            <div class="divide-y-2 divide-gray-100">
                                <template x-for="notif in filteredNotifications" :key="notif.id">
                                    <div @click="markAsRead(notif.id)"
                                        ...
                                        {{-- Icon --}}
                                        <div :class="!notif.is_read ? 'bg-indigo-100 border-indigo-200' :
                                            ...
                                            <template x-if="notif.tipe === 'pengumuman'">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p :class="!notif.is_read ? 'font-bold text-gray-900' : 'font-medium text-gray-600'"
                                            ...
                                            <div class="mt-1.5 flex items-center gap-2">
                                                ...
                                                <template x-if="!notif.is_read">
                                                    <span class="text-xs font-bold text-indigo-600">• Baru</span>
                                                </template>
                                            </div>
                                        </div>
                                        ...
                                        <template x-if="!notif.is_read">
                                            <div
                                                class="mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full bg-indigo-600 shadow-sm shadow-indigo-300">
                                            </div>
                                        </template>
                        ...
                        <template x-if="filteredNotifications.length === 0">
                            <div class="flex flex-col items-center justify-center py-24 text-center">
                                <div class="relative mb-6">
                                    <div
                                        class="flex h-24 w-24 items-center justify-center rounded-3xl border-2 border-indigo-100 bg-gradient-to-br from-indigo-50 to-blue-100">
                                        <x-heroicon-o-bell-slash class="h-12 w-12 text-indigo-300" />
                                    </div>
                                </div>
            ...
            function notifikasiPage() {
                return {
                    ...
                    async markAsRead(id) {
                        ...
                        try {
                            const res = await fetch(`/koor-ta/notifikasi/${id}/read`, {
                                method: 'POST',
                                credentials: 'same-origin',
                                headers: {
                    ...
                    async markAllAsRead() {
                        ...
                        this.loading = true;
                        try {
                            const res = await fetch("{{ route('koor-ta.notifikasi.read') }}", {
                                method: 'POST',
                                credentials: 'same-origin',
                                headers: {
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
