    ...
    <div x-data="pengaturanDosenPage()" class="space-y-6">
        ...
        <div
            ...
            <div class="relative flex items-center gap-5">
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-widest text-emerald-300">Pengaturan Akun</p>
                    <h2 class="mt-1 text-2xl font-black text-white leading-tight">{{ $user->name }}</h2>
                    <p class="mt-1 text-sm text-emerald-200">{{ $user->email }}</p>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-4">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    <div
                        class="flex items-center gap-3 border-b-4 border-emerald-200 bg-gradient-to-r from-emerald-600 to-teal-700 px-5 py-4">
                        <div
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="rounded-xl border-2 border-emerald-100 bg-emerald-50 px-4 py-3">
                            <p class="text-xs font-bold uppercase tracking-widest text-emerald-400 mb-1">Role</p>
                            <p class="text-sm font-black text-emerald-800">Dosen Pembimbing</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-emerald-200">Perbarui nama dan email Anda</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('dosen.pengaturan.profile') }}" enctype="multipart/form-data"
                        ...
                        <div class="flex items-center gap-5 rounded-xl border-2 border-gray-100 bg-gray-50 p-4">
                            <div class="relative shrink-0">
                                @if ($user->avatar)
                                    <img src="{{ asset('storage/' . $user->avatar) }}" alt="Avatar"
                                        class="h-16 w-16 rounded-full border-4 border-emerald-100 object-cover shadow-sm">
                                ...
                                @else
                                    <div
                                        class="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-xl font-black text-white shadow-sm border-4 border-emerald-100">
                                        {{ strtoupper(substr($user->name, 0, 2)) }}
                                    </div>
                                @endif
                            </div>
                            <div class="flex-1">
                                ...
                                <label class="cursor-pointer">
                                    ...
                                    <span
                                        class="inline-flex items-center gap-2 rounded-xl border-2 border-emerald-300 bg-emerald-600 px-4 py-2 text-xs font-black text-white shadow-sm transition hover:bg-emerald-700">
                        ...
                        {{-- Role readonly --}}
                        <div>
                            <label class="mb-1.5 block text-sm font-bold text-gray-700">Role</label>
                            <input type="text" value="Dosen Pembimbing" disabled
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    ...
                    <form method="POST" action="{{ route('dosen.pengaturan.password') }}" class="p-6 space-y-5">
                        @csrf
                        @method('PUT')
            ...
            function pengaturanDosenPage() {
                return {
                    ...
                    async removeAvatar() {
                        ...
                        try {
                            const res = await fetch("{{ route('dosen.pengaturan.avatar.remove') }}", {
                                method: 'DELETE',
                                headers: {
