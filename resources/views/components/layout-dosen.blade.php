...
<head>
    ...
    <title>{{ $title ?? 'Dashboard' }} - Sistem Skripsi Dosen</title>
    ...
    {{-- Alpine Store: Notification Dosen --}}
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.store('notifDosen', {
                ...
                async fetch() {
                    try {
                        const res = await fetch("{{ route('dosen.notifikasi.data') }}", {
                ...
                async markAllRead() {
                    try {
                        const res = await fetch("{{ route('dosen.notifikasi.read') }}", {
                            method: 'POST',
                            credentials: 'same-origin',
                            headers: {
                ...
                showNotification() {
                    if ('Notification' in window && Notification.permission === 'granted') {
                        new Notification('Notifikasi Baru', {
                            ...
                            icon: '/favicon.ico',
                            tag: 'skripsi-notification-dosen'
                        });
                    }
                },
        ...
        window.addEventListener('beforeunload', () => {
            if (Alpine.store('notifDosen')) {
                Alpine.store('notifDosen').destroy();
            }
        });
    </script>
    ...
    <div x-data="{ mobileMenu: false, scrolled: false }" class="h-screen flex overflow-hidden">
        ...
        <aside :class="mobileMenu ? 'translate-x-0' : '-translate-x-full md:translate-x-0'"
            ...
            <div class="p-6 pb-2">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-indigo-300">Panel Dosen</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-3 border-t border-white/10">
                <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/10">
                    <div
                        class="w-9 h-9 bg-gradient-to-br from-emerald-400 to-emerald-500 rounded-lg
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <div class="sticky top-0 z-30 transition-all duration-300"
                ...
                <x-header-dosen>{{ $title ?? 'Dashboard' }}</x-header-dosen>
            </div>
            ...
            <main class="flex-1 overflow-y-auto main-scroll" @scroll="scrolled = $el.scrollTop > 10">
                ...
                {{-- Scroll to Top --}}
                <button x-cloak x-show="scrolled" x-transition:enter="transition ease-out duration-200"
