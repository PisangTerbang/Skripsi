...
$watch('$store.notifDosen.unread', (val, oldVal) => {
    if (val > oldVal && val > 0) {
        bellShake = true;
    ...
    <div class="mb-1">
        ...
        <div class="space-y-1">
            {{-- Dashboard --}}
            <x-nav-link-dosen route="dosen.dashboard" icon="home">
                Dashboard
            </x-nav-link-dosen>

            {{-- Pengajuan --}}
            <x-nav-link-dosen route="dosen.pengajuan" icon="document-text">
                Pengajuan Mahasiswa
            </x-nav-link-dosen>

            {{-- Manajemen Judul --}}
            <x-nav-link-dosen route="dosen.judul.index" icon="academic-cap">
                Manajemen Judul
            </x-nav-link-dosen>
    ...
    <div class="mb-1">
        ...
        <div class="space-y-1">
            {{-- Notifikasi --}}
            <x-nav-link-dosen route="dosen.notifikasi" icon="bell">
                <span :class="{ 'animate-bell-shake': bellShake }">Notifikasi</span>

                <x-slot:badge>
                    {{-- Loading Skeleton --}}
                    <span x-show="$store.notifDosen.loading" x-transition class="ml-auto">
                        <span class="block w-6 h-6 rounded-full bg-white/20 animate-pulse"></span>
                    </span>

                    {{-- Badge Counter --}}
                    <span x-cloak x-show="!$store.notifDosen.loading && $store.notifDosen.unread > 0"
                        ...
                        x-text="$store.notifDosen.unread > 99 ? '99+' : $store.notifDosen.unread"
            ...
            {{-- Pengaturan --}}
            <x-nav-link-dosen route="dosen.pengaturan" icon="cog-6-tooth">
                Pengaturan
            </x-nav-link-dosen>
        </div>
    </div>
