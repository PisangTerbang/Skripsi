...
<body class="bg-gradient-to-br from-slate-50 via-amber-50/30 to-slate-100 h-screen overflow-hidden antialiased">
    ...
    <div x-data="{ mobileMenu: false, scrolled: false }" class="h-screen flex overflow-hidden">
        ...
        <aside :class="mobileMenu ? 'translate-x-0' : '-translate-x-full md:translate-x-0'"
            class="fixed md:relative inset-y-0 left-0 w-64 h-full
                      bg-gradient-to-b from-amber-600 via-amber-700 to-orange-800
            ...
            <div class="p-6 pb-4">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 bg-white/15 rounded-xl flex items-center justify-center shadow-lg shadow-amber-900/20">
                        <x-heroicon-o-shield-check class="w-6 h-6 text-white" />
                    </div>
                    <div>
                        <h1 class="text-lg font-bold tracking-tight">Kepala Lab</h1>
                        <p class="text-[11px] text-amber-300">Validasi Judul TA</p>
                    </div>
                </div>
            </div>
            ...
            <div class="px-4 pb-3">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="group w-full flex items-center gap-3 px-4 py-2 rounded-xl text-amber-200 hover:text-red-300 hover:bg-red-500/10 transition-all duration-200 ease-out">
            ...
            <div class="p-3 border-t border-white/10">
                <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-white/10">
                    <div
                        class="w-9 h-9 bg-gradient-to-br from-amber-400 to-orange-500 rounded-lg flex items-center justify-center font-bold text-xs shadow-sm">
                        {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold truncate leading-tight">{{ auth()->user()->name }}</p>
                        <p class="text-[11px] text-amber-300 truncate leading-tight mt-0.5">{{ auth()->user()->email }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden">
            <div class="sticky top-0 z-30 transition-all duration-300"
                ...
                <x-header-kalab>{{ $title ?? 'Dashboard' }}</x-header-kalab>
            </div>
            <main class="flex-1 overflow-y-auto main-scroll" @scroll="scrolled = $el.scrollTop > 10">
                ...
                <button x-cloak x-show="scrolled" x-transition
                    ...
                    class="fixed bottom-6 right-6 w-10 h-10 bg-amber-600 hover:bg-amber-700 text-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center hover:scale-105 active:scale-95 z-20">
