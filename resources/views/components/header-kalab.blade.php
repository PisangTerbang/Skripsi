                ...
                <div x-cloak x-show="notifOpen" @click.away="notifOpen = false"
                    ...
                    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                        ...
                        <button x-show="unread > 0" @click="markAllRead()"
                            class="text-[11px] font-bold text-sky-600 hover:text-sky-700">Tandai dibaca</button>
                    </div>
                    ...
                    <div class="max-h-80 overflow-y-auto">
                        ...
                        <template x-for="item in items" :key="item.id">
                            <a :href="item.link || '{{ route('ka-lab.notifikasi') }}'"
                                class="flex items-start gap-3 px-4 py-3 border-b border-gray-50 hover:bg-sky-50/50 transition-colors">
                                <span class="mt-1.5 h-2 w-2 shrink-0 rounded-full"
                                    :class="item.is_read ? 'bg-gray-200' : 'bg-sky-500'"></span>
                    ...
                    <a href="{{ route('ka-lab.notifikasi') }}"
                        class="block px-4 py-2.5 text-center text-xs font-bold text-sky-600 hover:bg-sky-50 border-t border-gray-100">
...
<script>
    ...
    function bellNotifKaLab() {
        return {
            ...
            async markAllRead() {
                try {
                    await fetch("{{ route('ka-lab.notifikasi.read') }}", {
                        method: 'POST',
                        credentials: 'same-origin',
                        headers: {
