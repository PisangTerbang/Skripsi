...
<header class="px-6 py-4">
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            ...
            <div x-data="bellNotifProdi()" x-init="init()" class="relative">
                ...
                <div x-cloak x-show="notifOpen" @click.away="notifOpen = false"
                    ...
                    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                        ...
                        <button x-show="unread > 0" @click="markAllRead()"
                            class="text-[11px] font-bold text-violet-600 hover:text-violet-700">Tandai dibaca</button>
                    </div>
                    ...
                    <div class="max-h-80 overflow-y-auto">
                        ...
                        <template x-for="item in items" :key="item.id">
                            <a :href="item.link || '{{ route('prodi.notifikasi') }}'"
                                class="flex items-start gap-3 px-4 py-3 border-b border-gray-50 hover:bg-violet-50/50 transition-colors">
                                <span class="mt-1.5 h-2 w-2 shrink-0 rounded-full"
                                    :class="item.is_read ? 'bg-gray-200' : 'bg-violet-500'"></span>
                    ...
                    <a href="{{ route('prodi.notifikasi') }}"
                        class="block px-4 py-2.5 text-center text-xs font-bold text-violet-600 hover:bg-violet-50 border-t border-gray-100">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    <div
                        class="w-8 h-8 bg-gradient-to-br from-violet-500 to-purple-700 rounded-lg flex items-center justify-center text-white text-xs font-bold shadow-sm">
                        {{ $initials }}
                    </div>
                    <div class="hidden md:block text-left">
                        ...
                        <p class="text-[11px] text-gray-400 leading-tight">Program Studi</p>
                    </div>
                ...
                <div x-cloak x-show="open" @click.away="open = false"
                    ...
                    <div class="px-4 py-3 border-b border-gray-100">
                        ...
                        <p class="text-xs text-violet-600 font-medium mt-1">Program Studi Informatika</p>
                    </div>

                    {{-- Notifikasi Link --}}
                    <a href="{{ route('prodi.notifikasi') }}"
                        class="flex items-center gap-3 px-4 py-2 text-sm text-gray-600 hover:bg-violet-50 hover:text-violet-700 transition-colors">
...
<script>
    ...
    function bellNotifProdi() {
        return {
            ...
            async markAllRead() {
                try {
                    await fetch("{{ route('prodi.notifikasi.read') }}", {
                        method: 'POST',
                        credentials: 'same-origin',
                        headers: {
