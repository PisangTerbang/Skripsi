<x-layout-prodi title="Pengaturan Akun">
    ...
    <div x-data="pengaturanPage()" class="space-y-6">
        ...
        <div
            ...
            <div class="relative flex items-center gap-5">
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-widest text-violet-300">Pengaturan Akun</p>
                    <h2 class="mt-1 text-2xl font-black text-white leading-tight">{{ $user->name }}</h2>
                    <p class="mt-1 text-sm text-violet-200">{{ $user->email }}</p>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-4">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    <div
                        class="flex items-center gap-3 border-b-4 border-violet-200 bg-gradient-to-r from-violet-600 to-purple-700 px-5 py-4">
                        <div
                    ...
                    <div class="p-4 space-y-3">
                        ...
                        <div class="rounded-xl border-2 border-violet-100 bg-violet-50 px-4 py-3">
                            <p class="text-xs font-bold uppercase tracking-widest text-violet-400 mb-1">Role</p>
                            <p class="text-sm font-black text-violet-800">Program Studi (Kaprodi)</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-violet-200">Perbarui nama dan email Anda</p>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('prodi.pengaturan.profile') }}" enctype="multipart/form-data"
                        class="p-6 space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="flex items-center gap-5 rounded-xl border-2 border-gray-100 bg-gray-50 p-4">
                            <div class="relative shrink-0">
                                @if ($user->avatar)
                                    <img src="{{ asset('storage/' . $user->avatar) }}" alt="Avatar"
                                        class="h-16 w-16 rounded-full border-4 border-violet-100 object-cover shadow-sm">
                                ...
                                @else
                                    <div
                                        class="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-purple-600 text-xl font-black text-white shadow-sm border-4 border-violet-100">
                                        {{ strtoupper(substr($user->name, 0, 2)) }}
                                    </div>
                                @endif
                            </div>
                            <div class="flex-1">
                                ...
                                <label class="cursor-pointer">
                                    ...
                                    <span
                                        class="inline-flex items-center gap-2 rounded-xl border-2 border-violet-300 bg-violet-600 px-4 py-2 text-xs font-black text-white shadow-sm transition hover:bg-violet-700">
                        ...
                        <div class="border-t-2 border-gray-100 pt-4">
                            <button type="submit"
                                class="inline-flex items-center gap-2 rounded-xl border-2 border-violet-300 bg-violet-600 px-6 py-3 text-sm font-black text-white shadow-sm transition hover:bg-violet-700 hover:shadow-md">
                ...
                <div class="overflow-hidden rounded-2xl border-2 border-gray-200 bg-white shadow-md">
                    ...
                    <form method="POST" action="{{ route('prodi.pengaturan.password') }}" class="p-6 space-y-5">
                        @csrf
                        @method('PUT')
            ...
            function pengaturanPage() {
                return {
                    ...
                    async removeAvatar() {
                        ...
                        try {
                            const res = await fetch("{{ route('prodi.pengaturan.avatar.remove') }}", {
                                method: 'DELETE',
                                headers: {
